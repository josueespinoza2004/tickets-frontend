...
/**
 * Script post-build para copiar archivos necesarios a la carpeta tickets
 */
...
const destHtaccess = path.join(__dirname, 'tickets', '.htaccess');

// Copiar .htaccess a tickets/
if (fs.existsSync(sourceHtaccess)) {
  try {
    fs.copyFileSync(sourceHtaccess, destHtaccess);
    console.log('✅ .htaccess copiado a tickets/');
  } catch (error) {
